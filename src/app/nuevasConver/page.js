...
//FUNCIONA EL BUSCAR
"use client";
import { Button, Menu, MenuItem, IconButton, Chip, Typography } from "@mui/material";
import { useClientes } from "@/hooks/useClientes";
import { useState, useEffect } from "react";
import { DataGrid } from "@mui/x-data-grid";
import NuevosFilters from "../components/NuevosFilters";
...
export default function NCPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      clearTimeout(handler); // Limpia si se escribe antes de 500ms
    };
  }, [filters.search]);

  // 2️⃣ Fetch con debounce
  useEffect(() => {
    const fetchNC = async () => {
      setLoading(true);
      try {
        const params = new URLSearchParams({
          ...
          orderBy: sortModel[0]?.field || "creado_en",
          order: sortModel[0]?.sort || "asc",
          search: debouncedSearch.length >= 2 ? debouncedSearch : "", // ✅ solo buscar si escribe mínimo 2 letras
        });
    ...
    sortModel[0]?.field,
    sortModel[0]?.sort,
    debouncedSearch, // 🔥 escuchamos el valor debounced
    filters.responded,
  ]);
